...
{
    /// <summary>
    /// Display and animation of environment resource particles (grass, trees, sky) in BuildAShip scene.
    /// </summary>
    class BuildAShipResourceRenderer : MonoBehaviour, ISceneDependency
    {
        ...
        private void OnSemanticBufferUpdated(ContextAwarenessStreamUpdatedArgs<ISemanticBuffer> args)
        {
            ...
            Channel channel = currentResource.Channel;

#if UNITY_EDITOR
            // Use the grass channel for testing in editor.
            if (channel == Channel.natural_ground) channel = Channel.grass;
#endif
